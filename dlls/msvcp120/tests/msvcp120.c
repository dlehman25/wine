...
static void test__Fiopen(void)
{
    ...
    static const struct {
        ...
        { "C",     L"utf_\x00e4\x00cf\x00f6\x00df.txt" },
        { "de_DE", L"utf_\x00e4\x00cf\x00f6\x00df.txt" },
    };
    ...
    for(i=0; i<ARRAY_SIZE(tests); i++) {
        ...
        if(!WideCharToMultiByte(CP_ACP, 0, tests[i].wpath, -1,
                                apath, ARRAY_SIZE(apath), NULL, NULL)) {
        ...
        ok(!!f, "failed to read %s (%s) with locale %s\n",
            apath, wine_dbgstr_w(tests[i].wpath), tests[i].loc);
        p_fclose(f);
