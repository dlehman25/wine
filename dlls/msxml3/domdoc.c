...
static char *strip_leading_xml_decl(const char *str, int *plen) {
   ...
   */
  if(len > 2 && p[0] == '<' && p[1] == '?') {
      while(p < str+len && (p[0] != '?' || p[1] != '>'))
          p++;
      p += 2;
      while(p < str+len && isspace(*p))
          p++;
      len -= p-str;
  }
    ...
    IPersistStreamInit *iface, IStream *stream, BOOL clr_dirty)
{
    ...
    if(hr == S_OK)
    {
        DWORD count;
        int len = 0;
        char *str = NULL, *newstr;

        bstr_to_utf8(xmlString, &str, &len);
        newstr = strip_leading_xml_decl(str, &len);
        hr = IStream_Write( stream, newstr, len, &count );
        heap_free(str);
