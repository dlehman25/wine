...
INT CDECL _wcsicmp_l(const wchar_t *str1, const wchar_t *str2, _locale_t locale)
{
    ...
    if(!locinfo->lc_handle[LC_CTYPE]) {
        ...
        do
        {
            if(!LCMapStringEx(LOCALE_NAME_USER_DEFAULT, LCMAP_LOWERCASE, str1, 1, &c1, 1, &info, NULL, 0))
               c1 = *str1;
            if(!LCMapStringEx(LOCALE_NAME_USER_DEFAULT, LCMAP_LOWERCASE, str2, 1, &c2, 1, &info, NULL, 0))
               c2 = *str2;
            str1++;
            str2++;
        } while(c1 && (c1 == c2));
