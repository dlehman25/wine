                                  ...
                                  char **buffer, int unix_len, int pos, UINT disposition, BOOLEAN check_case )
{
    ...
    if (dcache_enabled && with_drive && ret >= 0)
    {
        NTSTATUS status;
        ...
        if (dcache_enabled && (io->u.Status == STATUS_SUCCESS))
        {
            if (access & DELETE)
            {
                if (options & FILE_DIRECTORY_FILE)
                    ...
                    dc_rm_file(unix_name, *handle);
            }
            else if (created)
                dc_add_file(unix_name);
        }
        free( unix_name );
    }
...
static inline WCHAR *dc_memrchrW(const WCHAR *ptr, WCHAR ch, size_t n)
{
    const WCHAR *end;
    WCHAR *ret = NULL;
...
static int dfile_new(const WCHAR *name, int len, dfile_t **pfile)
{
    ...
    *pfile = NULL;
    ...
    {
        short_len = hash_short_file_name(name, len, short_name);
...
static inline int dc_build_path(const dlisting_t *listing, const dfile_t *file, char *unix_path, int unix_len)
{
    ...
    unix_len -= len;
    unix_path += len;
    if ((len = ntdll_wcstoumbs(file->name, file->len, unix_path, unix_len, TRUE)) <= 0)
        return ENAMETOOLONG;
    unix_path[len] = 0;
...
static int dc_read_listing(dlisting_t *listing, const WCHAR *want, dfile_t **pfile)
{
    ...
    *pfile = NULL;
    ...
    rc = 0;
    is83 = dc_is_8dot3(want, lstrlenW(want));
    while ((de = readdir(dir)))
    {
        if (!strcmp(de->d_name, ".") || !strcmp(de->d_name, "..")) continue;
                                  ...
                                  const WCHAR *file, int file_len, NTSTATUS *status)
{
    ...
    unix_name[pos - 1] = '/';
    ret = ntdll_wcstoumbs(dfile->name, dfile->len, unix_name + pos, MAX_DIR_ENTRY_LEN, TRUE);
    unix_name[pos + ret] = 0;
...
static inline void dc_del_dir(const WCHAR *dir)
{
    ...
    /* skip to last separator.  the unix path already has normalized slashes */
    dir_len = lstrlenW(dir);
    if (!(slash = dc_memrchrW(dir, '/', dir_len)))
        return;
