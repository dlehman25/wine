...
static float (__cdecl *p_cargf)(_Fcomplex);
...
static void test_carg(void)
{
    ...
    cf = p__FCbuild(1.0f, 0.0f);
    f = p_cargf(cf);
    ok(f == 0.0f, "got %lf\n", f);

    cf = p__FCbuild(0.0f, 1.0f);
    f = p_cargf(cf);
    ok(f == (float)M_PI_2, "got %lf\n", f);

    cf = p__FCbuild(-1.0f, 0.0f);
    f = p_cargf(cf);
    ok(f ==(float) M_PI, "got %lf\n", f);
}
